  ...
  return (
    <section className="py-18 sm:py-20">
      <SectionHeading
        ...
        action={<Button href="/xidmetler" variant="secondary">Xidmətlər səhifəsi</Button>}
      />
      <div className="site-container grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        {serviceDetails.map((service) => (
          <div key={service.slug} className="premium-card p-6">
            <FeatureIcon icon={service.icon} />
            <div className="mt-5 text-[11px] uppercase tracking-[0.28em] text-[var(--color-accent)]">{service.summary}</div>
            <h3 className="mt-2 text-2xl tracking-[-0.03em] text-[var(--color-text)]">{service.title}</h3>
